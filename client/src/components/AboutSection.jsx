...
export const AboutSection = () => {
  ...
  const achievements = [
    ...
    { number: "7+",  label: t.about.stats.projects,  icon: <Calendar className="h-5 w-5" />,  suffix: "" },
    { number: "∞",   label: t.about.stats.curiosity, icon: <Sparkles className="h-5 w-5" />,  suffix: "" },
    { number: "10+", label: t.about.stats.tech,      icon: <Target className="h-5 w-5" />,    suffix: "" },
  ];

  const techStack = [
    { category: t.about.stackCategories.frontend, items: ["React", "Vue.js", "TypeScript", "JavaScript", "HTML", "Bootstrap", "GSAP"] },
    { category: t.about.stackCategories.others,   items: ["C", "C++", "Electron", "Haskell"] },
    { category: t.about.stackCategories.tools,    items: ["Vercel", "Figma", "Shuffle", "Notion", "Claude"] },
  ];
  ...
  const handleDownload = () => {
    ...
    link.href = '/CV-ELIE-ADETONA.pdf';
    link.download = 'CV-ELIE-ADETONA.pdf';
  ...
  const tabs = [
    { key: 'personal',     label: t.about.tabs.personal },
    { key: 'professional', label: t.about.tabs.professional },
    { key: 'approach',     label: t.about.tabs.approach },
  ];
      ...
      <div className="container mx-auto max-w-7xl relative">
        <div className="text-center mb-16 md:mb-20 px-2 sm:px-6">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-bold mb-4 sm:mb-6">
            <span className="bg-gradient-to-r from-foreground to-primary bg-clip-text text-transparent">{t.about.title1}</span>
            <span className="block text-primary animate-pulse">{t.about.title2}</span>
          </h1>
          <p className="text-base sm:text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed"
            dangerouslySetInnerHTML={{ __html: t.about.subtitle.replace(/<b>/g, '<span class="text-primary font-semibold">').replace(/<\/b>/g, '</span>') }}
          />
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 md:gap-12">
          <div className="xl:col-span-2 space-y-8">
              ...
              <div className="relative">
                <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8">
                  <div className="relative flex-shrink-0">
                    <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-2xl overflow-hidden border-4 border-primary/20 shadow-2xl transition-all duration-500 group-hover:border-primary/40 group-hover:scale-105 md:group-hover:scale-110 relative">
                      <img src="/profile.jpg" alt="Elie ADETONA" className="w-full h-full object-cover" />
                ...
                <div className="flex flex-col sm:flex-row border-b border-border mb-4 sm:mb-6">
                  {tabs.map(tab => (
                    <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                      className={`flex-1 py-2 sm:py-3 px-2 sm:px-4 text-sm sm:text-base font-medium transition-all duration-300 ${activeTab === tab.key ? 'text-primary border-b-2 border-primary' : 'text-muted-foreground hover:text-foreground'}`}>
                      {tab.label}
                    </button>
                  ))}
                </div>
                ...
                <div className="min-h-[100px] sm:min-h-[120px]">
                  <AnimatePresence mode="sync">
                    <motion.p key={`${activeTab}-${t.about.tabs.personal}`}
                      initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }}
                      className="text-sm sm:text-base md:text-lg text-muted-foreground leading-relaxed">
                      {t.about.tabContent[activeTab]}
                    </motion.p>
                  </AnimatePresence>
                </div>
              </div>
            ...
            <div className="bg-card/50 border border-border rounded-3xl p-6 sm:p-8 backdrop-blur-xl shadow-2xl transition-all duration-500 hover:shadow-3xl hover:border-primary/40 hover:bg-card/60">
              <h3 className="text-lg sm:text-2xl font-bold mb-4 sm:mb-6 flex items-center gap-2 sm:gap-3">
                <Code className="h-4 sm:h-6 w-4 sm:w-6 text-primary" />{t.about.techStack}
              </h3>
          ...
          <div className="space-y-6 sm:space-y-8">
            <div className="bg-card/50 border border-border rounded-3xl p-6 sm:p-8 backdrop-blur-xl shadow-2xl transition-all duration-500 hover:shadow-3xl hover:border-primary/40 hover:bg-card/60">
              <h3 className="text-lg sm:text-2xl font-bold mb-4 sm:mb-6 text-center">{t.about.workTogether}</h3>
              <div className="flex flex-col sm:flex-row sm:space-x-4 space-y-3 sm:space-y-0">
                <a href="#contact" className="flex-1 block w-full p-3 sm:p-4 bg-primary text-primary-foreground rounded-xl text-center font-semibold transition-all duration-300 hover:bg-primary/90 hover:scale-105 hover:shadow-lg group">
                  <div className="flex items-center justify-center gap-2 sm:gap-3"><User className="h-4 sm:h-5 w-4 sm:w-5 group-hover:scale-110 transition-transform duration-300" />{t.about.startProject}</div>
                </a>
                <button onClick={handleDownload} className="flex-1 block w-full p-3 sm:p-4 border border-border rounded-xl text-center font-semibold transition-all duration-300 hover:bg-accent hover:border-primary/30 hover:scale-105 hover:shadow-lg group">
                  <div className="flex items-center justify-center gap-2 sm:gap-3">
                    <Download className="h-4 sm:h-5 w-4 sm:w-5 group-hover:translate-y-0.5 transition-transform duration-300" />{t.about.downloadResume}
                  </div>
                </button>
              </div>
            ...
            <div className="bg-card/50 border border-border rounded-3xl p-4 sm:p-6 backdrop-blur-xl shadow-2xl transition-all duration-500 hover:shadow-3xl hover:border-primary/40 hover:bg-card/60">
              <h3 className="text-base sm:text-xl font-bold mb-3 sm:mb-4 flex items-center gap-2"><Star className="h-4 sm:h-5 w-4 sm:w-5 text-primary" />{t.about.whyChooseMe}</h3>
            ...
            <div className="bg-card/60 border border-border rounded-3xl p-4 sm:p-6 backdrop-blur-xl shadow-2xl transition-all duration-500 hover:shadow-3xl hover:border-primary/40">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-2 sm:gap-0 mb-2 sm:mb-3">
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <span className="font-semibold text-xs sm:text-sm">{t.about.available}</span>
                </div>
                <span className="text-xs sm:text-sm text-muted-foreground bg-green-500/10 text-green-600 px-2 py-1 rounded-lg">{t.about.forNewProjects}</span>
              </div>
              <div className="text-[10px] sm:text-xs text-muted-foreground text-center bg-background/50 rounded-lg p-1 sm:p-2">{t.about.responseTime}</div>
            </div>
          </div>
        </div>
      </div>
