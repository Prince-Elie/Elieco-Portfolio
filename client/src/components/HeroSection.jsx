import { MousePointerClick, Code, Award, Download, Shield, Zap, TrendingUp, Mail } from "lucide-react";
...
export const HeroSection = () => {
  ...
  const achievements = [
    { number: "M1",   label: t.hero.stats.level,     icon: <Shield className="h-3 w-3" /> },
    { number: "∞",    label: t.hero.stats.curiosity,  icon: <TrendingUp className="h-3 w-3" /> },
    { number: "7+",   label: t.hero.stats.projects,   icon: <Award className="h-3 w-3" /> },
  ...
  useEffect(() => {
    ...
    if (displayedCode.length < currentLine.length) {
      const t = setTimeout(() => setDisplayedCode(currentLine.slice(0, displayedCode.length + 1)), 30);
      ...
      const t = setTimeout(() => {
        if (currentCodeLine < codeSnippets.length - 1) {
          ...
          setTimeout(() => { setCurrentCodeLine(0); setDisplayedCode(""); }, 5000);
        }
      }, 800);
      return () => clearTimeout(t);
    }
  ...
  const handleViewResume = () => {
    window.open('/CV-ELIE-ADETONA.pdf', '_blank', 'noopener,noreferrer');
  };
  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center justify-center px-4 sm:px-6 lg:px-8 overflow-hidden bg-gradient-to-br from-background via-background/95 to-primary/10" ref={ref}>
      ...
      <div className="absolute inset-0 -z-10 overflow-hidden">
        ...
        {[...Array(12)].map((_, i) => (
          <motion.div key={i} className="absolute bg-gradient-to-r from-primary/10 to-purple-500/10 rounded-lg"
            style={{ width: Math.random() * 60 + 20 + 'px', height: Math.random() * 60 + 20 + 'px', left: Math.random() * 100 + '%', top: Math.random() * 100 + '%', rotate: Math.random() * 360 }}
            animate={{ y: [0, (Math.random() - 0.5) * 60], x: [0, (Math.random() - 0.5) * 40], opacity: [0.1, 0.25, 0.1], scale: [1, 1.1, 1] }}
            transition={{ duration: Math.random() * 6 + 4, repeat: Infinity, repeatType: 'reverse' }}
          />
        ))}
      ...
      <div className="container max-w-7xl mx-auto w-full mt-16 sm:mt-0">
        <motion.div className="flex flex-col lg:flex-row items-center justify-between gap-16 lg:gap-20" initial="hidden" animate={isInView ? "visible" : "hidden"} variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.25, delayChildren: 0.5 } } }}>
          ...
          <div className="flex-1 text-center lg:text-left max-w-2xl mx-auto lg:mx-0">
            {/* Open to Work badge */}
            <motion.div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-card/60 border border-border backdrop-blur-sm mb-8 shadow-sm"
              variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}>
              <div className="relative">
                <div className="w-2.5 h-2.5 bg-green-500 rounded-full animate-pulse" />
                <div className="absolute inset-0 w-2.5 h-2.5 bg-green-500 rounded-full animate-ping" />
              </div>
              <span className="font-semibold text-sm text-foreground">{t.hero.openToWork}</span>
            </motion.div>
            ...
            <motion.p className="text-lg sm:text-xl text-muted-foreground mt-6 leading-relaxed max-w-2xl"
              variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}
              dangerouslySetInnerHTML={{ __html: t.hero.description.replace('<b>', '<span class="text-primary font-semibold">').replace('</b>', '</span>') }}
            />

            <motion.div className="grid grid-cols-2 sm:grid-cols-4 gap-4 my-8"
              variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}>
            ...
            <motion.div className="mt-6 text-center lg:text-left"
              ...
              <div className="text-sm text-muted-foreground">
                🚀 <span className="text-primary font-semibold">{t.hero.available}</span>
              </div>
            </motion.div>
          </div>
            ...
            <div className="relative w-full max-w-md">
              <motion.div className="bg-background/90 border border-border rounded-2xl p-8 backdrop-blur-sm shadow-2xl w-full group hover:shadow-3xl transition-all duration-500" whileHover={{ y: -4 }} transition={{ type: "spring", stiffness: 400, damping: 25 }}>
                    ...
                    <div className="grid grid-cols-1 gap-1 h-full content-start">
                      {codeSnippets.map((line, index) => (
                        <div key={index} className={`min-h-[20px] flex items-start ${index < currentCodeLine ? 'opacity-100' : 'opacity-0'} ${index === currentCodeLine ? 'opacity-100' : ''} transition-opacity duration-150 ease-in-out ${line.includes("import") ? "text-purple-400 font-semibold" : line.includes("const") || line.includes("new") ? "text-blue-400 font-semibold" : line.includes("React") || line.includes("TypeScript") ? "text-cyan-400" : line.includes("//") ? "text-muted-foreground italic" : line.includes("await") || line.includes("connect") ? "text-yellow-400" : line.includes("'") ? "text-amber-400" : "text-foreground"}`}>
                          ...
                          {index === currentCodeLine ? (<>{displayedCode}<motion.span animate={{ opacity: [1, 0, 1] }} transition={{ duration: 0.8, repeat: Infinity }} className="ml-1 text-primary inline-block">▊</motion.span></>) : ''}
                          {line === '' && '\u200E'}
                        </div>
                      ))}
                    </div>
                ...
                <motion.div className="absolute -top-3 -left-3 bg-background/90 backdrop-blur-sm px-4 py-2 rounded-xl border border-border shadow-lg flex items-center gap-2" initial={{ scale: 0, rotate: -180 }} animate={{ scale: 1, rotate: 0 }} transition={{ delay: 1.5, type: "spring" }}>
                  ...
                  <span className="text-sm font-semibold text-foreground">{t.hero.solutions}</span>
                </motion.div>
                <motion.div className="absolute -bottom-3 left-1/2 transform -translate-x-1/2 bg-background/90 backdrop-blur-sm px-4 py-2 rounded-xl border border-border shadow-lg text-center" initial={{ scale: 0, y: 20 }} animate={{ scale: 1, y: 0 }} transition={{ delay: 2, type: "spring" }}>
                  <div className="text-xs font-mono text-muted-foreground">{t.hero.builtWith}</div>
                  <div className="text-sm font-bold text-foreground">{t.hero.modernTech ?? "Modern Tech"}</div>
                </motion.div>
              </motion.div>
            </div>
          </motion.div>
